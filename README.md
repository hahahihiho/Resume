...
| Name | Life | Email  |
| :----: | :----: | :----: |
|  조현명 | 1994 ~ | hahahihiho16@gmail.com  |


<table>
<tr><td><img src="img/20260129.3.jpg" height = 250></td>
<td><img src="img/20260129.2.jpg" height = 250></td>
...
* 파고들어 고민하는 집중력이 있습니다. 하지만 빠르게 리서치하고 대중이 좋아하는 것을 감각하는 것은 익숙하지 않습니다.
* 방법들에 대해 고민하고 논의하는 것을 좋아합니다. 하지만 방향성을 정하는 것엔 시간이 걸립니다.
* 팀을 서포팅 할 때 신나는 편입니다. 하지만 주변의 반대를 무릅쓰고 추진하고 리딩하는 것은 약합니다.
...
프로젝트 진행방식
- 금융 데이터를 데이터처리하고 리서치들의 로직(코드)을 통과하여 결과를 만들어내는 pipeline을 관리했습니다. scheduling과 dependency, pipeline 문제발생 대응을 했습니다.

- 예측/실제 수익률과 각종 지표들을 매일 계산해서 시각적데이터로 보여줘야해서 로직 설계와 자동화, 시스템화 하는 일이 많았습니다.

- 문제가 생길 시 팀원들이 모여 방향을 같이 논의했고, 거기서 나온 효율적 결론과 side-effect, 그리고 현실적 가능성 사이에서 치열하게 고민하며 구현방식을 정해나갔습니다.
...
- 블록체인 tx 통신, 데이터 수집, 가공, 저장 및 api(js, python, sql, redis, influxDB, rabbitMQ)
...
[프로젝트 진행방식]
- DeFi(swap, lending) 를 구현하기위해 다양한 프로토콜들을 분석했고(UniswapV2,V3, Aave, DAI, Frax, Compound, Curve 등), 내부로직과 코드를 백서랑 같이 분석했습니다. 저는 주로 수학적 로직과 가스비를 줄이기 위해 만들어진 세부 코드 분석을 맡았습니다.

- ETF와 같이 여러 자산들을 한번에 분산투자할 수 있는 smart contract를 만들려는 시도도 했었지만 여러가지 제약과 생길 문제점들을 커버하기에 많은 연산량과 가스비가 구현된 복잡도에 비해 효율적이지도, 단순하지도 않았습니다.

- WemixFi 를 런칭했습니다. 코인의 가격과 거래량 데이터를 블록체인네트워크에서 가져와 차트데이터로 만드는 일을 했습니다. 차트를 보여주는 방식에 따라 어떤타이밍의, 어떻게 계산/측정된 가격을 쓸 것인지가 중요했고, 특히나 변동성이 큰 암호화폐 시장에서 중요한 가격 변동성을 다 보여주기가 쉽지 않아 타협점에 대한 논의가 많이 필요했습니다. 가격과 거래량을 추적하기위해 swap event와 tx을 following 하는것도 필요해 Queue를 사용하기도 했고, 어떤 시간 구간을 요청하던 차트데이터를 잘 보여주고 싶어 효율적인 시계열 데이터를 다루는 DB도 시도했었으나 요청시 마다 raw데이터에 계산하는 것은 부하가 커서 보여줄 시간구간을 정책적으로 정하고 미리 계산해두어야 한다는 결론을 냈습니다. 마지막 데이터만 효율적으로 업데이트 하는 로직이나 업데이트 타이밍 그리고 cache와 잘 연동하기 위한 로직을 많이 고민했던 시기였습니다.

- AI끼리 소통하여 결과를 내는 코드를 만드는 시도를 했습니다. 그 과정에서 CrewAI라는 프레임워크도 사용했었습니다. 하지만 AI가 모든 내용을 반영하지 못하는 부분과 수정을 요청했을 때 원하는 방향으로 가지 않는 경우가 많아, 작은 prototype 이상으로 큰 프로젝트를 자동화 하기엔 어렵단 결론을 냈습니다.

- AI툴을 만들면서 중간중간 사람 개입의 필요성과 완전한 자율화보다는 정해진 동작을 수행하는 Tool의 필요성을 느끼고 API를 연동하는 MCP을 만들었습니다. MCP가 헷갈리지 않고 할 수 있는 동작의 수에 제한이 있었기에 API 마다 동작을 만들지, API설명서를 만들고 API요청을 만들어 설명서를 읽어 요청을 보내게 할지 등 다양한 구조에 대해 고민을 했었습니다.

- LLM API를 사용해서 운세를 내는 프로젝트의 백엔드를 맡았습니다. 현재 날짜와 사주데이터가 정확하지 않아 정확하게 계산하여 넣어주는 로직을 만들었고, LLM 이 주는 결과값을 다시 사용하기 위해서 결과 format에 대한 처리방법들을 고민했습니다.

- 객체인식 및 글자인식을 통해 번호판을 정확하게 읽어내는 프로젝트를 진행했었습니다. Open source 모델들에 추가 학습을 시켜 번호판을 찾아내고, 또 글자인식 모델을을 이용하여 글자를 인식시키는 pipeline의 구조를 설계, 개발했습니다. 이미지의 공통성에 비해 글자는 국가별로 달라 글자인식의 정확도를 올리는 것이 어려웠습니다. 여러 모델을 사용하여 통계내는 방식, 후처리를 이용하는 방식, 국가별로 따로 학습하여 다른 모델을 만들어 사용하는 방식 등 여러 시도를 했었습니다.
